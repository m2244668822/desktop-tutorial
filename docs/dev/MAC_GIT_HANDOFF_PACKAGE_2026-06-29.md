...
for step in manual.get("ui_steps", []):
    print(f"- manual step: {step}")
...
| manual execution | Run one controlled manual execution after preflight reports `ready_for_manual_execution`, then rerun preflight until it reports `ready_for_activation` |

Credential binding and execution history are intentionally local-machine state. Keep API keys and n8n credential IDs out of the source workflow spec. On Mac, re-import the hardened spec, bind credentials in the n8n UI, then rerun preflight. The preflight will prefer `n8n_database_workflow` bindings from the local n8n DB and verify credential metadata by id/name/type without reading encrypted credential data. It reads only execution metadata for the manual-run gate and does not read execution payload data.
...
6. Run `tools/foundation_goal_audit.py --allow-incomplete`; do not claim complete while n8n activation is blocked or waiting for manual execution evidence.
7. Fix n8n activation blockers, then run one controlled manual execution before enabling the workflow.
