...
class TrevorDataMigrationTests(unittest.TestCase):
    ...
    def test_migration_is_repeatable_deduplicated_and_preserves_source_role(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            first_conversations = store.read_json(destination_file, {})
            first_thread_ids = set(first_conversations)
        ...
        self.assertEqual(first['source_counts'], second['source_counts'])
        self.assertNotIn('trevor_runtime', second['source_counts'])
        self.assertEqual(first_thread_ids, set(conversations))
